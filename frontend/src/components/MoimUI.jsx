...
import { IHome, ICalendar, IBell, IUser } from './Icons.jsx';
import { useNotificationContext } from '../notification/NotificationContext.jsx';
...
export function Tag({ children, color = 'clay', size = 'sm' }) {
  const map = {
    ...
    coral: ['var(--tag-coral-bg)', 'var(--tag-coral)'],
...
export function TabBar({ safeBottom = 0 }) {
  ...
  const getActive = () => {
    ...
    if (p.startsWith('/profile')) return 'me';
    return 'home';
  };
  ...
  const tabs = [
    ...
    { id: 'notif', label: '알림', Icon: IBell, dot: unreadCount > 0 },
    ...
    <nav style={{
      ...
      <div style={{ display: 'flex', height: 60 }}>
        {tabs.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => navigate(TAB_ROUTES[t.id])} style={{
...
export function BottomSheet({ open = true, onClose = () => {}, children, anchored = true }) {
  if (!open) return null;
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: 'var(--shadow-3)', padding: '10px 20px 32px',
